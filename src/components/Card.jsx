...
export default function Card({ className = "", urlImage = "", urlLogo = "", nameCard = "", tags = [], title = "", info = "", url = "#" }) {

    const figureImg = urlImage ? urlImage : '/src/assets/img/Image.png';
    const logoImg = urlLogo ? urlLogo : '';
           ...
           <figure style={{ backgroundImage: `url(${figureImg})` }}></figure>
            <div className="card-body pt-14 px-0 pb-0">
                ...
                <div className="tags flex mb-3 gap-2">
                    {tags.map((tag, i) => (
                        <div key={i} className="tag-toggle">{tag}</div>
                    ))}
                </div>
